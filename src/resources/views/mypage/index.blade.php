...
<div class="container mt-24">
  ...
  @if($view === 'buy')
      ...
      @if($bought && $bought->count())
        ...
        <div class="mt-16">{{ $bought->links() }}</div>
      @else
      ...
      @if($trading && $trading->count())
        <div class="grid grid--mypage">
          @foreach($trading as $row)
            <a href="{{ route('trade.show', $row->purchase_id) }}" class="card">
              <div class="card__thumb">
        ...
        <div class="mt-16">{{ $trading->links() }}</div>
      @else
        <p class="mt-24 muted">取引中の商品はありません。</p>
      @endif
  ...
  @else
      ...
      @if($sold && $sold->count())
        ...
        <div class="mt-16">{{ $sold->links() }}</div>
      @else

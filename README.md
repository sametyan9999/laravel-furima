...
- 購入者が評価を送信 → **購入者側では取引完了となり一覧から消える**
- 出品者は **自分が評価を送信するまで一覧に残る**
- 出品者もチャット画面右上から取引完了ボタン → 評価可能
- 最新メッセージ順で自動ソート
- 入力保持は localStorage 方式で実装済み

### UI についての補足（取引チャット画面）

本アプリの取引チャット画面は、参考デザイン（ワイヤーフレーム）では
上部にロゴのみが配置されたシンプルなヘッダー構成になっています。

一方、本アプリでは ユーザーの操作性を高めるために意図的に UI を拡張 しています。
- 上部ヘッダーには 「検索バー」「マイページ」「ログアウト」「出品ボタン」 を残した構成にしており、アプリ全体でのページ移動がしやすいよう設計しています。
- そのため、参考画像とはヘッダーの構造が異なりますが、アプリの利便性向上を目的とした仕様上の違いです。

取引チャット機能自体の挙動（メッセージ表示・送信・編集・削除・取引完了ボタン・評価モーダルなど）は、参考デザインに準拠して実装されています。

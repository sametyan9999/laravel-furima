...
| 機能                           | 画面 / URL                        | 確認方法の例 |
|--------------------------------|-----------------------------------|-------------|
| 取引中商品の一覧表示（FN001）  | マイページ「取引中の商品」タブ `/mypage?view=trade` | 販売者A / B でログインし、マイページ上部のタブを「取引中の商品」に切り替えると、現在取引中の商品一覧が表示されます。 |
| 取引メッセージ件数バッジ（FN005） | マイページ「取引中の商品」タブ | 各商品画像の左上に赤い丸バッジが表示され、その取引に対する **未返信メッセージ数** を確認できます。タブの「取引中の商品」横にも、全取引の未返信メッセージ合計が表示されます。 |
| 取引チャット画面への遷移（FN002 / FN003） | 取引チャット画面 `/trade/{purchase}` | マイページ「取引中の商品」から商品カードをクリックすると、該当取引のチャット画面へ遷移します。チャット画面左側のサイドバーから、他の取引チャットへ切り替えられます。 |
| 取引チャットの投稿 / 画像送信（FN006〜FN009） | 同上 `/trade/{purchase}` | 画面下部のフォームから本文・画像を入力して送信すると、メッセージが時間順に表示されます。バリデーションエラー時はフォーム上部にメッセージが表示され、本文は入力内容が保持されます。 |
| メッセージ編集・削除（FN010 / FN011） | 同上 `/trade/{purchase}` | 自分が送信したメッセージには「編集」「削除」リンクが表示されます。編集後に再送信すると内容が上書きされ、削除すると「このメッセージは削除されました」と表示されます。 |
| 取引自動ソート（FN004）       | マイページ「取引中の商品」タブ / チャット画面サイドバー | 新しいメッセージが投稿された取引ほど上に表示されます（最新メッセージ日時の降順）。 |
| 取引評価モーダル表示（FN012 / FN013 / FN014） | 取引チャット画面 `/trade/{purchase}` | 購入者として「取引を完了する」ボタンを押すと評価モーダルが表示されます。★1〜5 を選択して送信すると取引が完了し、商品一覧へ遷移します。出品者側は、購入者が完了後にチャット画面を開くと評価モーダルが表示されます。 |
| 評価平均の表示（US002 / FN005） | マイページ上部（プロフィール部分） `/mypage` | 他ユーザーからの取引評価の平均値が「★」で表示されます。まだ評価がない場合は表示されません。小数点は四捨五入して 1〜5 の整数になります。 |
| 完了メールの確認（FN015 / FN016） | MailHog `http://localhost:8025` | 購入者として取引評価を送信すると、出品者宛に「取引が完了しました」メールが送信されます。MailHog を開き、宛先（sellerA / sellerB のメールアドレス）を確認してください。 |
